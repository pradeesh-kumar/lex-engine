...
public final class LexUtils {
   ...
   *
   * <p>This method takes a 2D array of integers as input and returns its serialized form as a byte
   * array. The serialization is done using a ByteBuffer, where each integer in the 2D array is
   ...
   *
   * <p>This method takes a byte array as input, compresses it using GZIP, and returns the
   * compressed data as a new byte array.
   *
   ...
   *
   * <p>This method first decodes the base64 string into a byte array, then decompresses it using
   * GZIP, and finally deserializes the resulting byte array back into a 2D array.
   *
   ...
   *
   * <p>This method reads the input byte array as a GZIP-compressed stream, decompresses it, and
   * returns the resulting uncompressed data as a new byte array.
   *
   ...
   *
   * <p>This method assumes that the input byte array contains the serialized form of a 2D array,
   * where each integer is represented as four bytes. It uses a ByteBuffer to read the integers from
   * the byte array and constructs a new 2D array with the specified number of rows and columns.
   *
   ...
   * @throws BufferUnderflowException if the byte array does not contain enough data to fill the 2D
   *     array
   */
